...
def hello_world():
    ...
    for i, (l, r) in enumerate(zip(imgs, results.xyxy)):
        ...
        Y = img.shape[0]
        X = img.shape[1]
        for idx, box in enumerate(r.tolist()):
            if int(box[-1]) == 15 and box[-2] >= 0.5:
                ...
                imgNP = np.moveaxis(imgNP, -1, 0)

                imgTS = torch.tensor(imgNP).float().unsqueeze(0)
                ...
                featuresI = featuresI.reshape(1,-1)
                ...
                Ap = Anet(features).squeeze().detach().tolist()
                Bp = sorted(list(zip(BREEDS, sm(Bnet(features)).squeeze().detach().tolist())), key=lambda x: x[1], reverse=True)
                ...
                cats.append({'bbox': box[:4], 'age': Ap, 'breed': Bp,'clip':clip, 'dim':[width,height,left,top,X,Y]})
        rl.append(cats)
